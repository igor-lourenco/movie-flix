...
import { requestBackendLogin, salvaDadosAutenticacao } from "util/requests";
...
const Login = () => {
  ...
  const { setDadosAutContexto } = useContext(ContextoAutenticacao);
  ...
  const onSubmit = (dadosForm: DadosForm) => {
    requestBackendLogin(dadosForm)
      .then((response) => {
        setTemErro(false);
        salvaDadosAutenticacao(response.data); //Salva no local storage
        setDadosAutContexto({
          autenticado: true
        });
        history.push("/movies");
